...
class Skill:
    ...
    def rollout(self, env, obs, skill_limit, collect_states=False, drop_success=False):
        ...
        if collect_states:
            return obs, reward, terminated, info, rollout_num, state_list, img_list
        return obs, reward, terminated, info, rollout_num
    ...
    def rollout(self, env, total_len, stop_id, collect_states=False, drop_success=False, fail_search=False, **kwargs):
        ...
        while cur_node is not None and cur_node.node_id != stop_id:
            ...
            if cur_skill.traj_len_limit > 0:
                ...
                if collect_states:
                    obs, reward, terminated, info, traj_len, new_states, new_imgs = cur_skill.rollout(env, obs, traj_len_limit, collect_states=True, drop_success=drop_success)
                    state_store += [np.expand_dims(state, 0) for state in new_states]
                    img_store += new_imgs
                else:
                    obs, reward, terminated, info, traj_len = cur_skill.rollout(env, obs, traj_len_limit, drop_success=drop_success)
                traj_id += traj_len
            # drop success
            info['drop'] = False
    ...
    def eval_rollout(self, env, total_len, collect_states=False, **kwargs):
        ...
        while cur_node.skill is not None:
            ...
            if traj_len_limit == 0:
                ...
                if collect_states:
                    obs, reward, terminated, info, traj_len, new_states, new_imgs = cur_skill.rollout(env, obs, traj_len_limit, collect_states=True, drop_success=True)
                    ...
                    obs, reward, terminated, info, traj_len = cur_skill.rollout(env, obs, traj_len_limit, drop_success=True)
