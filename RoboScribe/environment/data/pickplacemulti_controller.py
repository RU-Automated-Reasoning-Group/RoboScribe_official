...
def get_pickmulti_control(obs, relative_grasp_position=(0., 0., -0.02), workspace_height=0.1, dist_atol=3e-2, other_atol=1e-3, target_dist_atol=3e-2, gain=10, block_id=0, last_block=False, need_away=False):
    """
    Returns
    -------
    action : [float] * 4
    ...
    if check_block == block_num and not need_away:
        return np.array([0., 0., 0., -1.]), True
