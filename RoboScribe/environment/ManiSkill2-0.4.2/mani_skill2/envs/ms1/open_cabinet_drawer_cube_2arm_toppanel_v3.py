...
@register_env("OpenCabinetDrawerCube2ArmToppanel-v3", max_episode_steps=400)
class OpenCabinetDrawerCube2ArmToppanelEnv(OpenCabinetEnv):
    ...
    def __init__(self, task: str = "opendrawer",extract: str="None", solve_cube: int=1, reassign=False, *args,  **kwargs):
        self.task = task
        self.robot_init_qpos_noise=0.02
        self._extract = extract
        self.solve_cube = solve_cube
        self.reassign = reassign
        self.reassign_status = None
        self.reassign_id = 0
        ...
        elif self._extract in ["drawer", "drawer_cube"]:
    ...
    def _load_actors(self):
        ...
        # set reassign
        self.reassign_status = [self.cubeA, self.cubeB, self.cubeC]
        self.reassign_id = 0
    ...
    def render(self, mode="human"):
        if mode in ["human", "rgb_array"]:
            ...
            # self.cubeA.hide_visual()
            # self.cubeB.hide_visual()
    ...
    def get_full_obs(self):
        obs = OrderedDict(
            ...
            extra=self._get_obs_extra(full=True),
        )
    ...
    def _get_obs_extra_prev(self) -> OrderedDict:
    ...
    def _get_obs_extra(self, full=False) -> OrderedDict:
        obs = super()._get_obs_extra()
        if self._extract in ["cube"]:
            obs = OrderedDict(
                tcp_pose=vectorize_pose(self.tcp.pose),
            ...
            if self._obs_mode in ["state", "state_dict"]:
                ...
                if self.solve_cube > 1 or full:
                    obs.update(
                        # cube B
                        cubeB_pose=vectorize_pose(self.cubeB.pose),
                        tcp_to_cubeB_pos=self.cubeB.pose.p - self.tcp.pose.p,
                        cubeB_to_place_goal_pos=self.cubeB.pose.p - self.place_goal_site.pose.p,
                        cubeB_in_drawer=1 if self.check_cube_in_drawer(self.cubeB) else 0,
                    )
                    if self.solve_cube > 2 or full:
                        obs.update(
                            # cube C
                            cubeC_pose=vectorize_pose(self.cubeC.pose),
                            tcp_to_cubeC_pos=self.cubeC.pose.p - self.tcp.pose.p,
                            cubeC_to_place_goal_pos=self.cubeC.pose.p - self.place_goal_site.pose.p,
                            cubeC_in_drawer=1 if self.check_cube_in_drawer(self.cubeC) else 0,
                        )
        elif self._extract in ["drawer"]:
            pass
        else:
    ...
    def step(self, action):
        ...
        # try to reassign
        if self.reassign and self.reassign_id < len(self.reassign_status) \
            and self.check_cube_in_drawer(self.reassign_status[self.reassign_id]):
            self.reassign_id += 1
            self.reassign_cubeA()
