...
@register_env("OpenCabinetDrawerCube-v1", max_episode_steps=400)
class OpenCabinetDrawerCubeEnv(OpenCabinetEnv):
    ...
    _task_type = ["opendrawer","closedrawer","pickcube", "pickcubeshelf", "pickcubeground"]
    ...
    def _initialize_task(self):
        ...
        self._set_env_by_tasktype(self._task)
        
        # MIGHT BE USEFUL
        # print(self.target_link)
        # print(dir(self.target_link))
        # print(self.target_link.pose)
        # self.obj_goal_site.set_pose(Pose(self.agent.get_ee_coords().mean(0)))
        # print(trimesh.PointCloud(self.target_handle_pcd).centroid)
        # print(trimesh.PointCloud(transform_points(
        #     self.target_link.pose.to_transformation_matrix(), self.target_handle_pcd
        # )).centroid)
        
        # self.obj_goal_site.set_pose(Pose(trimesh.PointCloud(transform_points(
        #     self.target_link.pose.to_transformation_matrix(), self.target_handle_pcd
        # )).centroid))
        
        # print(self.target_handle_sdf)
        # print("EE_coords:")
        # print(self.agent.get_ee_coords_sample().reshape(-1, 3))
        # print(self.target_link.pose.to_transformation_matrix())
        # handle_pcd = transform_points(
        #     self.target_link.pose.to_transformation_matrix(), self.target_handle_pcd
        # )
        # print(handle_pcd)
        # disp_ee_to_handle = sdist.cdist(self.agent.get_ee_coords_sample().reshape(-1, 3), handle_pcd)
        # print(disp_ee_to_handle)
        # print(disp_ee_to_handle.reshape(2, -1))
        # print(disp_ee_to_handle.reshape(2, -1).min(-1))
        # print("CCCCC")
    
    def _initialize_actors(self):
        # set cube position to be at the center of anotehr drawer
        if self._task in ["pickcubeshelf"]:
            # cube on the shelf
            cube_xyz = np.hstack([[0.01, 0.37], 0.765])
        elif self._task in ["pickcubeground"]:
            # cube on the ground
            cube_xyz = np.hstack([[-0.5, 0.0], 0.02])
        else:
    ...
    def _set_env_by_tasktype(self, task):
        """
        ...
        if task in ["opendrawer"]:
            ...
            qpos = np.hstack([xy, ori, h, arm_qpos])
            self.agent.reset(qpos)
        elif task in ["pickcubeshelf", "pickcubeground"]:
            canbinet_pos = [0, 0, 0.0, 0.39]
            self.cabinet.set_qpos(canbinet_pos)
            rd = self._episode_rng.uniform(-0.05, 0.05, [2])
            # goal_center = np.array([-0.4 + rd[0],  0.2 + rd[1],  0.7])
            ...
            # Base orientation
            noise_ori = self._episode_rng.uniform(-0.05 * np.pi, 0.05 * np.pi)
            ori = np.array([1.5]) + noise_ori
            h = 1e-4
            arm_qpos = np.array([-1.5, -1.5, 0, -2, 0, 2.2, 0.75, 0.04, 0.04])
            qpos = np.hstack([xy, ori, h, arm_qpos])
            self.agent.reset(qpos)
    ...
    def _register_render_cameras(self):
        ...
        if self._task in ["pickcube", ]:
            ...
            # cam_cfg.p = pose.p
            # cam_cfg.q = pose.q
        elif self._task in ["pickcubeshelf"]:
            pose = look_at([0.5, 0.0, 1.2], [0.01, 0.37, 0.765])
            # pose = look_at([0.0, -2, 1.3], [-0.3, 0.0, 1])
            cam_cfg.p = pose.p
            cam_cfg.q = pose.q     
        elif self._task in ["pickcubeground"]:
            pose = look_at([-0.5, 0.5, 0.5], [-0.5, 0.0, 0.02])
            cam_cfg.p = pose.p
            cam_cfg.q = pose.q
            # cam_cfg.p = [-2, 0.3, 2]
    ...
    def check_cube_in_drawer(self):
        goal_center = np.array([-0.4,  0.2, 0.69])
        goal_range = [0.05, 0.05, 0.01]
        diff = abs(self.obj.pose.p-goal_center)
        if (goal_range-diff > 0).all():
            return True
        else:
            return False
