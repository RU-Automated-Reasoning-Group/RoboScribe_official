...
class FetchMultiEnv(robot_env.RobotEnv):
    ...
    def reset_gripper(self):
        cur_state = self.sim.get_state()
        cur_state.qpos[:15] = self.initial_state.qpos[:15]
        cur_state.qvel[:] = self.initial_state.qvel[:]
...
class FetchNPushEnv(FetchMultiEnv, gym_utils.EzPickle):
    def __init__(self, reward_type="sparse", num_objects=1, collisions=False, order=False, rand_order=False, **kwargs):
        if collisions:
    ...
    def _sample_goal(self):
        if self.order:
            def get_xs(num_objects):
                xs = [-0.05*(i+1) for i in range(num_objects//2)] + \
                     [0.0] + \
                     [0.05*(i+1) for i in range(num_objects-1-num_objects//2)]
                if self.rand_order:
                    np.random.shuffle(xs)
                return xs
            
            goals = np.zeros((0, 3))
            # currently only support 3 objects
            # to support more objects, should consider length of gripper arm 
            assert self.num_objects == 3
            xs = get_xs(self.num_objects)
            # get init goal
            goal = self.initial_gripper_xpos[:3] + self.np_random.uniform(
                -self.target_range, -0.03, size=3)
            goal += self.target_offset
            goal[2] = self.height_offset
            for x in xs:
                new_goal = goal.copy()
                new_goal[0] = x + new_goal[0]
                goals = np.concatenate((goals, new_goal[None]), 0)
    ...
    # def compute_reward(self, achieved_goal, goal, info):
    #     # only consider sparse reward if order mode
    #     if self.order:
    #         if self.rew_fail:
    #             return -1.
    #         d = goal_distance(achieved_goal, goal)
    #         check_ids = np.argsort(goal[:, 0])
    #         sparse_reward_check = (d <= self.distance_threshold).astype(np.float32)
    #         if np.sum(sparse_reward_check) == self.num_objects:
    #             return 0.
    #         elif np.sum(sparse_reward_check) > 0:
    #             for b_id in check_ids:
    #                 if sparse_reward_check[b_id] != 1.0:
    #                     self.rew_fail = True
    #                     break
    #             return -1.
    #         else:
