...
class FetchPickAndPlace(gym.Env):
    def __init__(self, seed, obs_only=False, render_mode='rgb_array', simple_obs=False):
        initial_qpos = {
            "robot0:slide0": 0.405,
            "robot0:slide1": 0.48,
            "robot0:slide2": 0.0,
        ...
        if self.obs_only:
            if self.simple_obs:
                self.observation_space = spaces.Box(
                    -np.inf, np.inf, shape=np.concatenate([obs['observation'], obs['desired_goal'][:3]]).shape, dtype="float32")
            else:
                self.observation_space = spaces.Box(
                    -np.inf, np.inf, shape=np.concatenate([obs['observation'], obs['desired_goal']]).shape, dtype="float32")
        else:
    ...
    def step(self, action):
        ...
        if self.obs_only:
            if self.simple_obs:
                return np.concatenate([obs['observation'], obs['desired_goal'][:3]]), reward, done, done, info
            else:
                return np.concatenate([obs['observation'], obs['desired_goal']]), reward, done, done, info
        else:
    ...
    def reset(self):
        ...
        if self.obs_only:
            if self.simple_obs:
                return np.concatenate([obs['observation'], obs['desired_goal'][:3]]), {}
            else:
    ...
    def render(self, width=3000, height=2000):
    ...
    def get_obs(self, block_idx=0):
        ...
        if self.simple_obs:
            if obs['observation'].shape[0] == 6:
                b_idx = block_idx * 3 + 3
            elif obs['observation'].shape[0] == 9:
                b_idx = block_idx * 3 + 5
            else:
                pdb.set_trace()
                print('what?')
