...
model_path = "fetch/table_cleanup_1_block.xml"
n_substeps = 20
...
# TODO
filename1 = './predicate_1.pkl'

if os.path.isfile(filename1):
    # File exists, proceed with loading
    with open(filename1, 'rb') as file:
        predicate1 = pickle.load(file)

filename2 = './predicate_2.pkl'

if os.path.isfile(filename2):
    # File exists, proceed with loading
    with open(filename2, 'rb') as file:
        predicate2 = pickle.load(file)
...
class FetchCleanUp(gym.Env):
    ...
    # stuck function
    def state_is_stuck(self):
        obs = self.obs
        state = obs['observation']
        state = state.reshape(1, -1)
        rst = predicate1.predict(state)
        # print(f"state is stuck ({rst})")
        if rst == 0:
            return False
        return True
    
    def gripper_is_stuck(self):
        obs = self.obs
        state = obs['observation']
        state = state.reshape(1, -1)
        rst = predicate2.predict(state)
        # print(f"state is stuck ({rst})")
        if rst == 0:
            return False
        return True
