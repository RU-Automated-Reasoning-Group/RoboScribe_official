...
To generate Figure.20 in the paper, we collect the evaluation success rate for single tower, pyramid and multi-tower environments and hard code them in the file ```transfer_data_visualize/plot.py``` (We will provide the code to load the evaluation data from ```store/tree_tower5_transfer/debug_skill/transfer_eval_result.npy``` for visulization in the later version). User could enter the directory and run the plot file to generate the figure:
```
